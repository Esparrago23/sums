...
export class InMemoryEstiloVidaRepository implements IEstiloVidaRepository {
  async create(estiloVida: EstiloVida): Promise<EstiloVida> {
    const query = `
      INSERT INTO estilo_vida (persona_id, toxicomanias, actividad_fisica, alimentacion, higiene_personal)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *;
    `;
    const values = [
  ...
  async update(estiloVida: EstiloVida): Promise<EstiloVida> {
    const query = `
      UPDATE estilo_vida
      SET persona_id = $1, toxicomanias = $2, actividad_fisica = $3, alimentacion = $4, higiene_personal = $5
      WHERE id = $6
      RETURNING *;
    `;
    const values = [
